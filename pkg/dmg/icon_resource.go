...
func iconResourceFork(source macfs.Source, icon []byte) ([]byte, error) {
	...
	if d < 16 || m < 16 || dl < 0 || ml < 28 || d > len(raw)-dl || m > len(raw)-ml {
		return bad()
	}
	data := append([]byte(nil), raw[d:d+dl]...)
	rm := raw[m : m+ml]
	tl, nl := u16(rm[24:]), u16(rm[26:])
	if tl < 28 || tl > len(rm)-2 || nl > len(rm) {
		return bad()
	}
	count := u16(rm[tl:]) + 1
	if count > 4096 || tl+2+count*8 > len(rm) {
		return bad()
	}
	...
	for i := 0; i < count; i++ {
		...
		n := u16(entry[4:]) + 1
		off := tl + u16(entry[6:])
		if n > 4096 || off < tl || off > len(rm)-n*12 {
			return bad()
		}
		typ := resourceType{name: string(entry[:4])}
		for j := 0; j < n; j++ {
			var ref [12]byte
			copy(ref[:], rm[off+j*12:off+(j+1)*12])
	...
	for _, typ := range types {
		refsSize += len(typ.refs) * 12
	}
	nameOffset := 28 + 2 + len(types)*8 + refsSize
	if nameOffset > 65535 {
		return nil, fmt.Errorf("resource map is too large")
	}
	newMap := make([]byte, nameOffset)
	copy(newMap[22:24], rm[22:24])
	binary.BigEndian.PutUint16(newMap[24:], 28)
	binary.BigEndian.PutUint16(newMap[26:], uint16(nameOffset))
	binary.BigEndian.PutUint16(newMap[28:], uint16(len(types)-1))
	offset := 2 + len(types)*8
	...
	binary.BigEndian.PutUint32(out[4:], uint32(256+len(data)))
